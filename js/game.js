...
//A game asset for the levels in the game.
function Levels(game) {
	...
	this.receiveKey = function(keyCode) {
		if (keyCode === 82) {
			levels.levelIndex = 0;
			levels.level = 1;
		}
...
function Sprite(game) {
	...
	this.h = 0;											//Phat Parrot is a squat little fellow.
	...
	for (var i = 0; i < this.imageNames.length; i++) {	//Load all the image frames
		...
		if (i === 0) {										//When loading the first image.			
		...
		//Set the image's source file name.
...
function Gates(game) {
	...
	this.gateTop.src = "img/gateTop.png";		//Top gate image file name.
	this.gateBottom.src = "img/gateBottom.png";	//Bottom gate image file name.
	...
	this.update = function(game) {
		...
		for (var i = 0; i < NUM_GATES; i++) { //Iterate the array of gates.
			...
			if (!this.collision && gate.x < (game.ctx.canvas.width / 2) && !gate.scored) {
				...
				gate.scored = true; //Mark this gate as stats.
			}
...
function Stats(game) {
	...
	this.x = ((game.ctx.canvas.width / 2) - 240);	//Set stats to middle of screen horizontally.
